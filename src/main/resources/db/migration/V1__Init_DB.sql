create table correct_gaz_answer
(
    id             int8 generated by default as identity,
    correct_answer varchar(4000),
    primary key (id)
);
create table correct_heat_answer
(
    id             int8 generated by default as identity,
    correct_answer varchar(4000),
    primary key (id)
);
create table correct_indust_answer
(
    id          int8 generated by default as identity,
    name_answer varchar(4000),
    primary key (id)
);
create table correct_indust_answer_correct_answer
(
    correct_indust_answer_id int8 not null,
    correct_answer           varchar(4000)
);
create table gaz_questions
(
    id     int8 generated by default as identity,
    answer varchar(255),
    name   varchar(4000),
    primary key (id)
);
create table gaz_questions_new
(
    id   int8 generated by default as identity,
    name varchar(4000),
    primary key (id)
);
create table gaz_questions_test_options
(
    gaz_questions_id int8 not null,
    test_options     varchar(4000)
);
create table heat_questions
(
    id     int8 generated by default as identity,
    answer varchar(255),
    name   varchar(4000),
    primary key (id)
);
create table heat_question_test_options
(
    heat_question_id int8 not null,
    test_options     varchar(4000)
);
create table industrial_questions
(
    id     int8 generated by default as identity,
    answer varchar(255),
    name   varchar(4000),
    primary key (id)
);
create table industrial_questions_test_options
(
    industrial_questions_id int8 not null,
    test_options            varchar(4000)
);
alter table if exists correct_indust_answer_correct_answer
    add constraint FKn6viq0w82hfh3mo7hons9encx foreign key (correct_indust_answer_id) references correct_indust_answer;
alter table if exists gaz_questions_test_options
    add constraint FKjh6sa4g96wsttda0vbf4nvghq foreign key (gaz_questions_id) references gaz_questions;
alter table if exists heat_question_test_options
    add constraint FKah6mqoif0hfl2cepfbi6r2y5i foreign key (heat_question_id) references heat_questions;
alter table if exists industrial_questions_test_options
    add constraint FKg7s77gor49xouw5lbhkkrfmn foreign key (industrial_questions_id) references industrial_questions;
;